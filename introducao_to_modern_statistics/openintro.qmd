...
# Proporcao, em cada genero, de Homens e Mulheres. (versao do Livro)
ucb_admit %>%
  group_by(gender) %>%
  count(admit) %>%
  mutate(prop_h_m = n/sum(n))

## Proporcao Homens e Mulheres em Relacao ao Total. (Minha Versao)
ucb_admit %>%
  count(admit, gender) %>% # Usar group_by dificultaria a proporcao total.
  mutate(prop_h_m_admitidos = n/sum(n))
  
# Proporcao Homens e Mulheres Admitidos em Relacao ao Total.
ucb_admit %>%
  count(gender, admit) %>%
...
# Proporcao Homens Admitidos em cada Departamento.(Minha Versão)
ucb_admit %>%
